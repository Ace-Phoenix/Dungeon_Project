...
//alot more than without it being a class
  class ClassGrid extends Dungeon{
    constructor (name = "x", sizey = 200, sizex = 150, grid = [], number = 200){
      super(name)
      this._name = name;
       this._sizey = sizey;
       this._sizex = sizex;
...
//Function draw uses grid to make a border and draw the grid itself
  draw(grid, sizey, sizex){
   // grid.size = 300
   // grid.sizex = 100
   // grid.sizey = 200
   var grid = this.grid(ClassGrid, sizey, sizex);
...
//stil do not knoiw why it dose not go through that second for statement for the rooms
  ...
  //Function room uses numbers, grid, element and name to make rooms randomly
    ...
    var grid = this.grid(ClassGrid, name = "x");
...
var grid = classs.grid(element = ClassGrid, name = "x");
classes = new ClassGrid(name = "x",sizey = 200 , sizex = 150, grid, number = 200)
console.log(classes);
  ...
  document.getElementById("Rooms").onclick = function(){
    for (var i = 0; i < 10; i++) {
      ...
      document.getElementById("blah").innerHTML = classes.room(ClassGrid, name=" ");
}
